...
var checkNumInputs = function checkNumInputs(selector) {
  var numInputs = document.querySelectorAll(selector); // проверка инпутов на номер
...
var forms = function forms(state) {
  ...
  var windows = document.querySelectorAll("[data-modal]"); // проверка инпутов на номер телефона
  ...
  form.forEach(function (item) {
    item.addEventListener("submit", function (e) {
      e.preventDefault(); // создаем блок статуса сообщения

      var statusMessage = document.createElement("div");
      statusMessage.classList.add("status");
      item.appendChild(statusMessage); //собираем данные из формы
      ...
      item.addEventListener("click", function (e) {
        ...
        if (modal.classList.contains("popup_calc_profile")) {
          if (!state.form || !state.width || !state.height) {
            alert("Выберите/заполните все данные");
            return;
          }
        }
